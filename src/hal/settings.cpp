...
void showSettings() {
    ...
    logRaw("    serialJsonConsole: %s", serialJsonConsole ? "true" : "false");
...
void loadSettings() {
    ...
    serialJsonConsole  = prefs.getBool("jsonConsole", false);
    ...
    #ifdef NRF52_PLATFORM
    oledPageInterval = prefs.getUChar("oledPageIv", 5);  // stored as seconds on nRF52
    oledPageInterval *= 1000;
    #else
...
void saveOledSettings() {
    ...
    #ifdef NRF52_PLATFORM
    prefs.putUChar("oledPageIv", (uint8_t)(oledPageInterval / 1000));
    #else
...
void saveSettings() {
    ...
    prefs.putBool("jsonConsole", serialJsonConsole);
