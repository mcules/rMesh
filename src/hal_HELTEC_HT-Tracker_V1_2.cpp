...
void initHal() {
    ...
    //Vext einschalten (TFT + GNSS Stromversorgung)
    pinMode(PIN_VEXT_CTRL, OUTPUT);
    digitalWrite(PIN_VEXT_CTRL, HIGH);  // HIGH = Vext an

    //Ausgaenge
    pinMode(PIN_WIFI_LED, OUTPUT);
    digitalWrite(PIN_WIFI_LED, 0);
    pinMode(PIN_VBAT_CTRL, OUTPUT);
    digitalWrite(PIN_VBAT_CTRL, LOW);   // Spannungsteiler standardmaessig aus
...
void transmitFrame(Frame &f) {
    ...
    // Duty-Cycle-Check fuer Public-Band (10 % in 60 s)
    if (isPublicBand(settings.loraFrequency)) {
        ...
        if (!dutyCycleAllowed(toa)) {
            Serial.println("[LoRa] Duty-Cycle-Limit erreicht, TX uebersprungen.");
            return;
        }
